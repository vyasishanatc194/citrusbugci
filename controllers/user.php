...
//           }
	...
	public function deletecsv(){
		echo '<pre>';
		$sql = "select member_id from `red_members`";
		$rsusername_exists=$this->db->query($sql);
		if($rsusername_exists->num_rows()>0){
			
			foreach($rsusername_exists->result_array() as $recmsg => $mid){
				
				$memberid = $mid['member_id'];
				$user_dir = $memberid % 1000;
				$upload_path = $this->config->item('user_private') . $user_dir . '/' . $memberid . '/csv_files/';
				...
				foreach($files as $fileName)
				{
					...
					{
						unlink($this->config->item('user_private') . $user_dir . '/' . $memberid . '/csv_files/'.$fileName);
